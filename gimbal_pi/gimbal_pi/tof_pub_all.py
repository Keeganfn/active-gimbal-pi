        ...
        try:
            while rclpy.ok():
                ...
                if frame != None:
                    ...
                    depth_buf_raw = np.array(depth_buf)
                ...
                amp_msg = self.bridge.cv2_to_imgmsg(amplitude_buf.astype(np.uint8), "8UCI")
                colorized_msg = self.bridge.cv2_to_imgmsg(result_image.astype(np.uint8), "8UCI")
